MODULE gort_lidar
  ...
  ! fp            : foliage profile
  ! afp           : accumulated foliage profile
  ...
  type profile_params
     ...
     real, dimension(:), pointer :: afp           ! accumulated foliage profile, no clump involved
...
contains
  ...
  subroutine run_single_gort(gin, height_levels, G, gamma, fp, efp, afp, pgap, dpdz, wvfm) 
    ...
    real, dimension(:), pointer   :: efp            ! effective foliage profile: gamma*foliage profile
    real, dimension(:), pointer   :: afp            ! accumulated foliage profile
    real, dimension(:), pointer   :: pgap(:,:)           ! gap probability 
    real, dimension(:), pointer   :: dpdz(:,:)           ! dp/dz
    real, dimension(:), pointer   :: wvfm(:,:)           ! waveform
    ...
    ! get variables, clump, fp, afp and efp
    ...
    ! fp = G *fp  ! fp does not have G WN - 6/19/2024
    efp =  gamma * fp
    ! get accumulated foliage profile - WN 6/19/2024
    afp = acfp(fp, delta_z)

    ! compute Pgap
    allocate(Pgap(N_height_level,2))
    Pgap(:,1) = T(fp*G, gin%dz, gin%zenith)
    Pgap(:,2) = T(efp*G, gin%dz, gin%zenith)
    ...
    !compute waveform  (Equation (3) in Ni-Meister et al., 2011) by WNM 1/19/2011 and eqn (7) in Ni-Meister et al. 2018
 ...
 subroutine run_convolute_gort(gin, height_levels, G, gamma, fp, efp, afp, pgap, dpdz, wvfm) 
   ...
   real, dimension(:), pointer   :: height_levels  ! the height levels for the profile
   real, dimension(:), pointer   :: fp             ! foliage profile
   real, dimension(:), pointer   :: efp            ! effective foliage profile: amma*foliage profileg
   real, dimension(:), pointer   :: afp            ! accumulated foliage profileg
  ...
  ! get accumulated foliage profile - WN 6/19/2024
    afp = acfp(fp, delta_z)/G(1)  !not exactly correct, but the best we can do WN 6/19/2024

   ! compute Pgap
   allocate(pgap(N_convolute_height_level,2))
   pgap(:,1) =  T(fp, dz, szn)   ! no need G factor here, different from single GORT run WN 6/19/2024
   pgap(:,2) =  T(efp, dz, szn) 
...
! ----------------------------------------------------------------------------------------------------
 function acfp(fp, delta_z)

   ! return a acculated foliage profile based on the size of input fp

   real, dimension(:), pointer :: fp
   real :: delta_z

   real, dimension(size(fp)) :: afp
   ...
   N_height_level = size(fp)
   do ilevel=1, N_height_level
     afp(ilevel) = -sum(fp(ilevel:N_height_level))*delta_z
   end do
