pro run_gort
 ...
 tek_color
  ...
  device, file= 'rw_fb.eps', /TIMES,/BOLD,/portrait, ysize=28, YOFFSET=0, xsize=21, xOFFSET=0.0
  !p.multi=[0,3,3]
...
;    print,ilevel
  ...
  if (i eq 1) then begin
        plot, gwvfm, ght,title=strmid(fplot(i-1),63,17),xtitle='Normalized WF', ytitle='Height(m)', xrange=[jx1,jx2], yrange=[jh1,jh2],charsize=cs,charthick=ct
      endif else begin            
        plot, gwvfm, ght,title=strmid(fplot(i-1),63,17),xtitle='Normalized WF', ytitle='Height(m)', xrange=[jx1,jx2], yrange=[jh1,jh2],charsize=cs,charthick=ct
      endelse
