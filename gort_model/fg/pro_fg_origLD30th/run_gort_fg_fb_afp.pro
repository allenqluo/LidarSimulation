...
indir = '/data/shared/src/STV/LidarSimulation/OrigGORTInput/2024_06_26/guiana/origLD30th/with_branch/*.in'
...
;read gort output file 
  ...
  readf, 2, temp_str
  readf, 2, temp_str1
  print, strmid(fplot(i-1),88+11,3), ' ', temp_str, temp_str1
...
; print, total(fp,/CUMULATIVE)*dz
  ...
  plot, total(fp,/CUMULATIVE)*dz, ght,title='FG/B+L/'+ strmid(fplot(i-1),88+11,3),xtitle='Accumulated FP', ytitle='Height(m)', xrange=[jx1,jx2], yrange=[jh1,jh2],charsize=cs,charthick=ct
endfor
DEVICE,/CLOSE 
stop
end
