...
;read gort output file 
  ...
  readf, 2, temp_str
  readf, 2, temp_str1
  print, strmid(fplot(i-1),63,17), ' ', temp_str, temp_str1
...
;  plot, total(fp,/CUMULATIVE)*dz, ght,title=strmid(fplot(i-1),64,17),xtitle='Accumulated FP', ytitle='Height(m)', xrange=[jx1,jx2], yrange=[jh1,jh2],charsize=cs,charthick=ct
  plot, total(fp,/CUMULATIVE)*dz, ght,title='RW/B+L/'+strmid(fplot(i-1),78,3),xtitle='Accumulated FP', ytitle='Height(m)', xrange=[jx1,jx2], yrange=[jh1,jh2],charsize=cs,charthick=ct
endfor
DEVICE,/CLOSE 
stop
end
