pro run_gort
 ...
 tek_color
  ...
  device, file= 'rw_fb_afp.eps', /TIMES,/BOLD,/portrait, ysize=28, YOFFSET=0, xsize=21, xOFFSET=0.0
  !p.multi=[0,3,3]

nsite = 9
;indir = '/data/shared/src/STV/LidarSimulation/OrigGORTInput/redwood/with_branch/*.in'
indir = '/data/shared/src/STV/LidarSimulation/OrigGORTInput/2024_06_26/redwood/orig/with_branch/*.in'
...
;read gort output file 
  ...
  readf, 2, temp_str
  readf, 2, temp_str1
  print, strmid(fplot(i-1),78+16,3), ' ', temp_str, temp_str1
...
;  plot, total(fp,/CUMULATIVE)*dz, ght,title=strmid(fplot(i-1),64,17),xtitle='Accumulated FP', ytitle='Height(m)', xrange=[jx1,jx2], yrange=[jh1,jh2],charsize=cs,charthick=ct
  plot, total(fp,/CUMULATIVE)*dz, ght,title='RW/B+L/'+strmid(fplot(i-1),78+16,3),xtitle='Accumulated FP', ytitle='Height(m)', xrange=[jx1,jx2], yrange=[jh1,jh2],charsize=cs,charthick=ct
endfor
DEVICE,/CLOSE 
stop
end
